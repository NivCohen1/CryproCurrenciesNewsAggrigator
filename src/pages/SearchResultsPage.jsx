...
const SearchResultsPage = () => {
    ...
    const fetchNews = async () => {
      try {
        ...
        const response = await fetch(
          `https://min-api.cryptocompare.com/data/v2/news/?categories=${crypto}&api_key=YOUR_API_KEY&_=${new Date().getTime()}`
        );
  ...
  // Filter news based on the crypto word in the title
  useEffect(() => {
    if (news.length > 0) {
